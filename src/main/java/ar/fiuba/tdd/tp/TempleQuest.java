...
public class TempleQuest extends Game {
    ...
    public TempleQuest() {
        ...
        switch1.addAction(new TriggerActionHandler(switch1, new BeTalkedTo(switch1), switch1ActivadoTriggers, switch1ActivadoTriggeredValues));
        ...
        switch2.addAction(new TriggerActionHandler(switch2, new BeTalkedTo(switch2), switch2ActivadoTriggers, switch2ActivadoTriggeredValues));
    ...
    public boolean checkLooseCondition() {
        ...
        if ((estabaEnShore1TurnoAnterior.isFalse() && estaEnShore1Ahora.isTrue()) || (estabaEnShore1TurnoAnterior.isTrue() && estaEnShore1Ahora.isFalse())) {
            if (player.getChildren().size() > 1) { // no puedo tener mas de un item cuando cruzo la soga, si no muero
                return true;
            }
        }

        return acantilado.contains(player.getName()) || (room4.contains(player.getName()) && (poisoned.isTrue() || !arqueologo.contains(disc9.getName())));
    }
}
